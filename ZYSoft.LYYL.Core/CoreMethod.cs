    ...
    public class CoreMethod : BaseMethod
    {
        ...
        public string GetMORounting(string v_str_Json)
        {
            try
            {
                ...
                if (_DAPI_MO.FModID.Contains("-"))
                {
                    v_str_sql = string.Format(@"select b.status, b.moid, b.modid, a.mocode,b.sortseq,b.invcode,c.cinvname,c.cinvstd,d.cComUnitName,b.Free1 cVersion, 
                ...
                else if (dt_MO.Rows[0]["status"].ToString().Equals("1"))
                {
                    return returnResult("", "生产订单未审核,禁止报工", null);
                }
                else if (dt_MO.Rows[0]["status"].ToString().Equals("4"))
                {
                    return returnResult("", "生产订单已关闭,禁止报工", null);
                }
            ...
            ", _DAPI_MO.FModID);
                ...
                v_str_sql = string.Format(@"SELECT row_number() over (order by opseq) as ino,  MoDId,MoRoutingId,t1.MoRoutingDId,OpSeq,t1.OperationId,Description,WcId,Remark,  FirstFlag,LastFlag,BFFlag, SubFlag,SubQty,
                ...
                foreach (DataRow dr in dt_Process.Rows)
                {
                    ...
                    _MORoutingEntry.IsSubFlag= bool.Parse(dr["SubFlag"].ToString());

                    if (decimal.Parse(dr["SubQty"].ToString()) > 0)
                    {
                        _MORoutingEntry.IsSubIssueFlag = true;
                    }
                    ...
                    //超额报工后未报为负数时直接显示0
                    if (_MORoutingEntry.iUnRptQty < 0)
                    {
                        _MORoutingEntry.iUnRptQty = 0;
                    }
                    ...
                    foreach (DataRow dr_Setp in dr_SetpS)
                    {
                        ...
                        {
                            //取上道工序的合格数量
                            _MORoutingSetpEntry.iPlanQty = decimal.Parse(dr_Find[0]["iGoodQty"].ToString());
                            //_MORoutingEntry.iPlanQty = decimal.Parse(dr["iPlanQty"].ToString());
                        }
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.WriteErrLog(0, ex.StackTrace.Substring(ex.StackTrace.LastIndexOf("\\") + 1, ex.StackTrace.Length - ex.StackTrace.LastIndexOf("\\") - 1));
                        ...
                        if (_DAPI_MORpt.FIsLastFlag)
                        {
                            ...
                            if (_DAPI_MORpt.FOpName.Equals("车间流转"))
                            {
                                ...
                                v_str_sql = string.Format(@"INSERT INTO [{0}].[dbo].[t_PDARecord]
                                    ...
                                    (case when t2.bInvBatch=1 THEN '2' ELSE '' END),'{10}',0,ISNULL(T1.Qty,0) - ISNULL(T1.QualifiedInQty,0),  0 ,
                                ...
                                else
                                {
                                    ...
                                    DataSet ds1 = ZYSoft.DB.BLL.Common.GetDataSetByTime(v_str_sql, 60 * 1000);

                                    if (ds1 == null)
                                    {
                                        //生单失败的情况下将记录删除
                                        v_str_sql = "DELETE FROM [ZYSoft_LYYL_2019].[dbo].[t_MOReportList] WHERE  FGuid = '" + v_str_Guid + "' ";
                                        ZYSoft.DB.BLL.Common.ExecuteNonQuery(v_str_sql);

                                        return returnResult("", "生成产成品入库失败", null);
                                    }

                                    string v_str_U8Result1 = ds1.Tables[0].Rows[0]["FResult"].ToString();

                                    if (v_str_U8Result1 != "Y")
                                    {
                                        //生单失败的情况下将记录删除
                                        v_str_sql = "DELETE FROM [ZYSoft_LYYL_2019].[dbo].[t_MOReportList] WHERE  FGuid = '" + v_str_Guid + "' ";
                                        ZYSoft.DB.BLL.Common.ExecuteNonQuery(v_str_sql);

                                        return returnResult("", "生成产成品入库失败," + v_str_U8Result1, null);
                                    }
        ...
        public string DelMORpt(string v_str_Json)
        {
            try
            {
                ...
                {
                    #region 删除时，如果是末道工序，且工序是车间流转 时 将自动删除产成品入库单
                    //如果是末道工序，且 工序是 车间流转 的 那么 直接删除 产成品入库单
                    //select * from [dbo].[t_MOReportList] where [FIsLastFlag]=1  and FOpName='车间流转'
                    if (dt_Record.Rows[0]["FOpName"].ToString().Equals("车间流转") &&
                       bool.Parse(dt_Record.Rows[0]["FIsLastFlag"].ToString()) )
                    {
                        //取出生成的入库单据ID
                        v_str_sql = "select isnull(FBuildBillID,'') FBuildBillID from  [ZYSoft_LYYL_2019].[dbo].t_PDARecord where FIdentifier = '" + dt_Record.Rows[0]["FGuid"].ToString() + "'";

                        string v_str_BillID = ZYSoft.DB.BLL.Common.ExecuteScalar(v_str_sql);
                        if (!v_str_BillID.Equals(""))
                        {
                            v_str_sql = "EXEC [ZYSoft_LYYL_2019].[dbo].P_ZYSoft_CallU8Service  'Del_MORdReocrd', '" + v_str_BillID + "' ";
                            string v_str_U8Result2 = ZYSoft.DB.BLL.Common.ExecuteScalar(v_str_sql);

                            if (v_str_U8Result2 != "Y")
                            {
                                return returnResult("", "删除产成品入库失败," + v_str_U8Result2, null);
                            }
                        }
                    }
                    #endregion
                    ...
                    FROM ZYSoft_LYYL_2019.dbo.t_MOReportQty T1 JOIN ZYSoft_LYYL_2019.dbo.t_MOReportList t2 on t1.FMOID = T2.FMOID
                    AND T1.FMoDId = T2.FMoDId AND T1.FMoRoutingDId = T2.FMoRoutingDId AND T1.FOpSeq = T2.FOpSeq AND T1.FStepCode=T2.FStepCode
                    WHERE T2.FID = '{0}' AND T2.FIsBuildFC=0 AND T2.FIsVerifyFlag=0 AND T2.FIsNeedConfirm=1 ", _DAPI_DelMORpt.FID);
                    ls_Sql.Add(v_str_sql);
                    ...
                    //修复异常数据  如果更新后记录为负数，直接删除  
                    v_str_sql = string.Format(@"DELETE FROM ZYSoft_LYYL_2019.dbo.t_MOReportQty 
                    WHERE FRptQty<0  AND FMoDId = {0} ", dt_Record.Rows[0]["FMoDId"].ToString());
                    ls_Sql.Add(v_str_sql);
        ...
        public string GetGridConfig(string v_str_GridNo)
        {
            try
            {
                ZYSoft.DB.Common.Configuration.ConnectionString = connStr;
                DataTable table = ZYSoft.DB.BLL.Common.ExecuteDataTable(string.Format(@"select [FTableName],[FEntryID]
                ,t2.[FID] ,[FNo],[FFieldName],[FFieldCaption],[FWidth],[FAlign],[FFiexed],[FDateType],[FVisible]
                ,ISNULL([FIsFilter],0)[FIsFilter],ISNULL([FIsSectionFilter],0)[FIsSectionFilter],ISNULL([FDBType],1)[FDBType]
                from t_CustomRpt t1 left join  t_CustomRptEntry t2 on t1.FID = t2.FID 
                where ISNULL(t2.FVisible,0) =1 AND t1.[FReportNo] = '{0}' order by FNo ", v_str_GridNo));
                if (table == null)
                {
                    return BaseMethod.returnResult("", "未获取到网格配置信息", null);
                }
                if (table.Rows.Count == 0)
                {
                    return BaseMethod.returnResult("", "未获取到网格配置信息", null);
        ...
        /// <param name="v_str_Json"></param>
        /// <returns></returns>
        public string GetGridData(string v_str_Json)
        {
            try
            {
                ZYSoft.DB.Common.Configuration.ConnectionString = connStr;
                DataTable table = ZYSoft.DB.BLL.Common.ExecuteDataTable(v_str_Json);
                if (table == null)
                {
                    return BaseMethod.returnResult("", "未获取到数据", null);
                }
                if (table.Rows.Count == 0)
                {
                    return BaseMethod.returnResult("", "未获取到数据", null);
        ...
        /// <param name="v_str_Json">传入工单号、登录用户</param>
        /// <returns></returns>
        public string GetMOProcess(string v_str_Json)
        {
            try
            {
                //LogHelper.WriteErrLog(0, v_str_Json); 
                ...
                string v_str_ErrMsg = "";
                //解析
                DAPI.DAPI_MOProcess _DAPI_MO = JsonConvert.DeserializeObject<DAPI.DAPI_MOProcess>(v_str_Json);
                ...
                //根据工单分录ID，取出工单及产品信息
                Regex rx = new Regex("^[0-9]*$");
                if (!rx.IsMatch(_DAPI_MO.FValue))
                {
                    if (!_DAPI_MO.FValue.Contains("-"))
                    {
                        return returnResult("", "生产订单录入有误", null);
                    }
                }

                if (_DAPI_MO.FType.Equals("0"))
                {
                    if (_DAPI_MO.FValue.Contains("-"))
                    {
                        v_str_sql = string.Format(@"select b.moid, b.modid, a.mocode,b.sortseq,d.PartId, c.cinvcode,c.cinvname,c.cinvstd,b.MoLotCode ,B.Define32 cProject
                        from mom_order a with(nolock) inner join mom_orderdetail b  with(nolock) on a.moid = b.moid
                        inner join inventory c on b.invcode = c.cinvcode 
                        JOIN dbo.bas_part d ON c.cInvCode=d.InvCode
                        where  a.mocode = '{0}' and b.sortseq ='{1}'
                        ", _DAPI_MO.FValue.Split('-')[0], _DAPI_MO.FValue.Split('-')[1]);
                    }
                    else
                    {
                        v_str_sql = string.Format(@"select b.moid, b.modid, a.mocode,b.sortseq,d.d.PartId,c.cinvcode,c.cinvname,c.cinvstd,b.MoLotCode ,B.Define32 cProject
                         from mom_order a with(nolock) inner join mom_orderdetail b  with(nolock) on a.moid = b.moid
                        inner join inventory c on b.invcode = c.cinvcode 
                        JOIN dbo.bas_part d ON c.cInvCode=d.InvCode
                        where b.modid = '{0}'
                ...
                {
                    v_str_sql = string.Format(@"select  c.cinvcode,c.cinvname,c.cinvstd,d.PartId
                from inventory c JOIN dbo.bas_part d ON c.cInvCode=d.InvCode  WHERE c.cinvcode  = '{0}'
                        ", _DAPI_MO.FValue);
                }
                DataTable dt_MO = ZYSoft.DB.BLL.Common.ExecuteDataTable(v_str_sql);
                ...
                v_str_sql = string.Format(@"select b.Picture,b.cPicturetype from inventory a 
                                inner join AA_Picture b on a.PictureGUID=b.cGUID  where a.cinvcode='{0}' AND b.Picture IS NOT NULL ",
                            dt_MO.Rows[0]["cinvcode"].ToString());
                ...
                else if (dt_Pic.Rows.Count > 0)
                {
                    ...
                    v_str_FileName = string.Format(@"{0}/{1}", _DAPI_MO.FUrl, dt_MO.Rows[0]["cinvcode"].ToString() + "." + dt_Pic.Rows[0]["cPicturetype"].ToString());
                    //LogHelper.WriteErrLog(0, v_str_FileName);
                }
                ...
                //只取最大版本号的工艺路线
                v_str_sql = string.Format(@" SELECT  MAX(t1.Version) Version
                            FROM sfc_prouting t1 JOIN sfc_proutingpart t2 on t1.PRoutingId = t2.PRoutingId
                            where T2.PartId = {0}", dt_MO.Rows[0]["PartId"].ToString());
                string v_str_Version = ZYSoft.DB.BLL.Common.ExecuteScalar(v_str_sql);

                v_str_sql = string.Format(@"SELECT row_number() over (order by opseq) as ino,t1.Version   , 
                    t1.PRoutingId, t3.PRoutingDid,t3.OpSeq,t3.OperationId,t3.DESCRIPTION ,t3.Remark       
                    FROM  sfc_prouting t1 JOIN sfc_proutingpart t2 on t1.PRoutingId=t2.PRoutingId
                    JOIN   sfc_proutingdetail t3 ON t1.PRoutingId=t3.PRoutingId                     
                    where T2.PartId= {0} AND t1.Version='{1}'  ORDER BY opseq ", dt_MO.Rows[0]["PartId"].ToString(), v_str_Version);
                DataTable dt_Process = ZYSoft.DB.BLL.Common.ExecuteDataTable(v_str_sql);

                if (dt_Process == null)
                {
                    return returnResult("", "未获取到工艺路线", null);
                }
                else if (dt_Process.Rows.Count == 0)
                {
                    return returnResult("", "未获取到工艺路线记录", null);
                }

                //取出工步 
                v_str_sql = string.Format(@"SELECT T1.PRoutingDid,T1.OperationId,T2.cStepCode,cStepName
                    FROM [HM_LY_PRountingStepMain] T1 JOIN [HM_LY_PRountingStepSub] T2 ON T2.ID = T1.ID    
                    LEFT JOIN  sfc_proutingdetail T3 ON T1.PRoutingDId=T3.PRoutingDId
                    LEFT JOIN      sfc_prouting T4 ON T3.PRoutingId=T4.PRoutingId
                    LEFT JOIN    sfc_proutingpart T5 ON T4.PRoutingId=T5.PRoutingId     
                    WHERE  T5.PartId= {0} AND t4.Version='{1}'  ORDER BY T1.OperationId,t2.cStepCode ", 
                    dt_MO.Rows[0]["PartId"].ToString(), v_str_Version);
                DataTable dt_Setp = ZYSoft.DB.BLL.Common.ExecuteDataTable(v_str_sql);
                ...
                //取出图纸信息
                v_str_sql = string.Format(@" EXEC ZYSoft_LYYL_2019.dbo.[P_ZYSoft_CallProcessLookUpService] 'FindProcessFile','{0}' ", dt_MO.Rows[0]["cinvcode"].ToString());
                DataTable dt_ProcessFile = ZYSoft.DB.BLL.Common.ExecuteDataTable(v_str_sql,ref v_str_ErrMsg);

                if (dt_ProcessFile == null)
                {
                    return returnResult("", "未检查到工艺文件," + v_str_ErrMsg, null);
                }
                else if (dt_ProcessFile.Rows.Count == 0)
                {
                    return returnResult("", "未检查到工艺文件", null);
                }
                ...
                DAPI.DAPI_MOQueryProcess _MORouting = new DAPI.DAPI_MOQueryProcess();
                ...
                _MORouting.cInvCode = dt_MO.Rows[0]["cinvcode"].ToString();
                _MORouting.cInvName = dt_MO.Rows[0]["cinvname"].ToString();
                _MORouting.cInvStd = dt_MO.Rows[0]["cinvstd"].ToString(); 
                _MORouting.cInvImgPath = v_str_FileName;
                //_MORouting.cInvImgPath = string.Format(@"{0}/{1}", _DAPI_MO.FUrl, dt_MO.Rows[0]["invcode"].ToString() + "." + dt_Pic.Rows[0]["cPicturetype"].ToString());

                List<DAPI.DAPI_MOQueryProcessRouting> ls_MOQueryProcessRouting = new List<DAPI.DAPI_MOQueryProcessRouting>();

                foreach (DataRow dr in dt_Process.Rows)
                {
                    #region 工序
                    DAPI.DAPI_MOQueryProcessRouting _MORoutingEntry = new DAPI.DAPI_MOQueryProcessRouting();
                    ...
                    ls_MOQueryProcessRouting.Add(_MORoutingEntry);
                    ...
                    //如果工序有工步，则按工步最小的合格数量作为此工序的可汇报数量
                    DataRow[] dr_SetpS = dt_Setp.Select("OperationId= '" + dr["OperationId"].ToString() + "' and PRoutingDid = '" + dr["PRoutingDid"].ToString() + "'");


                    foreach (DataRow dr_Setp in dr_SetpS)
                    {
                        DAPI.DAPI_MOQueryProcessRouting _MORoutingSetpEntry = new DAPI.DAPI_MOQueryProcessRouting();

                        _MORoutingSetpEntry.SortSeq = dr_Setp["cStepCode"].ToString();
                        _MORoutingSetpEntry.ProcessName = dr_Setp["cStepName"].ToString();
                        _MORoutingSetpEntry.ProcessDesc = "";
                        _MORoutingSetpEntry.IsSetp = true;

                        ls_MOQueryProcessRouting.Add(_MORoutingSetpEntry);
                    }

                    #endregion
                }

                _MORouting.MOQueryProcessRouting = ls_MOQueryProcessRouting;

                List<DAPI.DAPI_MOQueryProcessFile> ls_MOQueryProcessFile = new List<DAPI.DAPI_MOQueryProcessFile>();
                foreach (DataRow dr in dt_ProcessFile.Rows)
                {
                    #region 工艺文件
                    DAPI.DAPI_MOQueryProcessFile _MOProcessFile = new DAPI.DAPI_MOQueryProcessFile();

                    DataRow[] dr_Find = dt_Process.Select("ino = '" + (int.Parse(dr["ino"].ToString()) - 1).ToString() + "'");

                    _MOProcessFile.FileName = dr["FileName"].ToString();
                    _MOProcessFile.FilePath = dr["FilePath"].ToString();

                    ls_MOQueryProcessFile.Add(_MOProcessFile);

                    #endregion
                }
                _MORouting.MOQueryProcessFile = ls_MOQueryProcessFile;
